...
abstract class WaicTable {
			...
			if ($id) {
				...
				foreach ($data as $field) {
					...
					foreach ($field as $k => $v) {
						if (isset($this->_fields[$k])) {
							$row[$k] = waicToeCreateObj('WaicField', array(
								$this->_fields[$k]->name,
								$this->_fields[$k]->html,
								$this->_fields[$k]->type,
	...
	public function getTable( $transform = false ) {
		if ($transform) {
			return WaicDb::prepareQuery($this->_table);
		} else {
			return $this->_table;
		}
	}
	...
	public function get( $fields = '*', $where = '', $tables = '', $return = 'all' ) {
		...
		return WaicDb::get($query, $return);
	}
	public function store( $data, $method = 'INSERT', $where = '' ) {
		...
		if (empty($fields)) {
			$this->_addError(esc_html__('Nothing to update', 'ai-copilot-content-generator'));
			return false;
		}
			...
			foreach ($data as $k => $v) {
				if (array_key_exists($k, $this->_fields) || $k == $this->_id) {
					...
					if (isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) {
						$val = WaicFieldAdapter::_($val, $this->_fields[$k]->adapt['dbTo'], WaicFieldAdapter::DB);
					}
					if ($validate) {
						if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
							$objForValidation = clone $this->_fields[$k];
							$objForValidation->setValue($val);
							$errors = WaicValidator::_($objForValidation, $this);
							if ($errors) {
	...
	/**
	 * Add new FieldWaicWaic for children table (@see class field)
	 *
	...
	protected function _addField( $name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '' ) {
		$this->_fields[$name] = waicToeCreateObj('WaicField', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
	/**
			...
			switch ($f->type) {
				case 'date':
					if ('0000-00-00' == $d[$key] || empty($d[$key])) {
						...
						$d[$key] = gmdate(WAIC_DATE_FORMAT, WaicDb::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
	...
	public function getLastInsertID() {
		return WaicDb::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
	}
