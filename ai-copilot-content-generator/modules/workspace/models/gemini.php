...
class WaicGeminiModel extends WaicModel implements WaicAIProviderInterface {
	...
	public function getApiEmbeddingsUrl() {
		return $this->apiUrl . '/models/' . $this->model . ':embedContent?key=' . $this->apiKey;
	}
	...
	private function sendRequest( $url, $method = 'POST', $params = array(), $type = '' ) {
		...
		} else if ('embeddings' == $type) {
			if (isset($data->embedding) && isset($data->embedding->values)) {
				$results['error'] = 0;
				$results['data'] = array(array('embedding' => $data->embedding->values));
			} else {
				$results['msg'] = 'Embedding not get';
			}
	...
	public function sendEmbeddings( $params, $method = 'POST' ) {
		$this->model = WaicUtils::getArrayValue($this->apiOptions, 'model');
		$url = $this->getApiEmbeddingsUrl();
		
		$this->geminiParams = array(
			'model' =>'models/' . $this->model,
			'content' => array('parts' => array(array('text' => empty($params['input']) ? '' : $params['input']))),
		);
		$dimension = WaicUtils::getArrayValue($this->apiOptions, 'dimension');
		if (!empty($dimension)) {
			$this->geminiParams['output_dimensionality'] = $dimension;
		}
